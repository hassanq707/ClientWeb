...
import { FaCar, FaMotorcycle, FaTruck, FaShip, FaShieldAlt, FaCertificate } from 'react-icons/fa';
import { IoMdSpeedometer } from 'react-icons/io';
import { GiCarKey } from 'react-icons/gi';
...
const InfoSection = ({ currentTypeIndex, currentText }) => {
  ...
  return (
    <div className="text-white px-2 mt-0 md:mt-6 md:px-6 lg:px-10 text-left w-full">
      <div className="flex items-center mb-6">
        <div className="p-3 rounded-xl border-4 border-blue-800/70 shadow-lg">
          {getVehicleIcon()}
        </div>
        <h1 className="text-4xl md:text-5xl lg:text-5xl font-bold ml-4">
          <span className="text-white">Fusions</span>
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-300 to-blue-400"> Car</span>
        </h1>
      </div>
      
      <p className="text-xl md:text-2xl mb-8 font-light">
        Instant verification for your <span className="font-medium">{currentText}</span>
      </p>
      ...
      <ul className="space-y-3  text-lg">
        <li className="flex items-start">
          ...
          <span className='mt-1'>Accident & Damage History with severity analysis</span>
        </li>
        <li className="flex items-start">
          <div className="bg-blue-700/30 p-2 rounded-lg mr-3 ">
            <GiCarKey className="text-blue-300 text-xl" />
          </div>
          <span className='mt-1'>Complete ownership records & title verification</span>
        </li>
        <li className="flex items-start">
          <div className="bg-blue-700/30 p-2 rounded-lg mr-3 ">
            <IoMdSpeedometer className="text-blue-300 text-xl" />
          </div>
          <span className='mt-1'>Mileage verification to detect rollback fraud</span>
        </li>
        <li className="flex items-start">
          <div className="bg-blue-700/30 p-2 rounded-lg mr-3 ">
            <FaCertificate className="text-blue-300 text-xl" />
          </div>
          <span className='mt-1'>Certified reports accepted by dealers worldwide</span>
        </li>
      </ul>

      <div className="mt-10 flex flex-wrap gap-4">
        <div className="px-5 py-3 rounded-xl border-2 lg:border-4 border-blue-800/70 shadow-lg">
          <div className="text-blue-200 font-bold text-xl">10M+</div>
          <div className="text-sm text-blue-100/80">Records</div>
        </div>
        <div className="px-5 py-3 rounded-xl border-2 lg:border-4 border-blue-800/70 shadow-lg">
          <div className="text-blue-200 font-bold text-xl">98%</div>
          <div className="text-sm text-blue-100/80">Accuracy</div>
        </div>
        <div className="px-5 py-3 rounded-xl border-2 lg:border-4 border-blue-800/70 shadow-lg">
          <div className="text-blue-200 font-bold text-xl">24/7</div>
